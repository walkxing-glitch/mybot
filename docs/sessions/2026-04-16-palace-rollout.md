...
- **10 个 phase 全部完成**，commit chain：
  ...
  ```
- **测试：** palace 测试 79/79 green（torch 相关的 embedder/reranker 测试跳过，见下）。
...
```bash
# palace 全量测试
$ pytest tests/palace/ -q --deselect tests/palace/test_embedder.py --deselect tests/palace/test_reranker.py
79 passed
